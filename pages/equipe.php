...
<tr>
<?php $dados = listarVisitasACS($ano, $mes, $filtros); 
...
<tr>
  ...
  <small>
      <?= htmlspecialchars($row['no_equipe'] ?? '-') ?> -
      <?= htmlspecialchars($row['no_profissional'] ?? '-') ?>
      <br>

      <!-- ✅ MICROÁREA -->
      Microárea:
      <strong><?= htmlspecialchars($row['nu_micro_area'] ?? '-') ?></strong>
    </small> - QTD:<?php 
	$filtros1 = [];
	$filtros1['cnes'] = explode(',', $row['nu_cnes']);
    $filtros1['equipe'] = $row['nu_ine'];
    $filtros1['microarea'] = $row['nu_micro_area'];
