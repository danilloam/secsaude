...
// ==========================================
// Só usamos o cache geral se NÃO houver filtros específicos na URL
if (empty($_GET['cnes']) && empty($_GET['equipe']) && empty($_GET['microarea']) && empty($_GET['acs']) && $distritoId) {
    ...
    if (file_exists($arquivoCache)) {
        ...
        if (isset($cacheCompleto['visitas'])) {
            echo json_encode([
                'status' => 'success',
                'source' => 'cache',
                'data'   => $cacheCompleto['visitas']
...
try {
    error_log('FILTROS VISITAS (BANCO): ' . print_r($filtros, true));
    ...
    error_log('ERRO API VISITAS: ' . $e->getMessage());
