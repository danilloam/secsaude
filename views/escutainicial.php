...
if (empty($_GET['cnes']) && empty($_GET['equipe']) && empty($_GET['microarea']) && empty($_GET['acs']) && $distritoId) {
    ...
    if (file_exists($arquivoCache)) {
        ...
        if (isset($cacheCompleto['escutainicial'])) {
            echo json_encode([
                'status' => 'success',
                'source' => 'cache',
                'data'   => $cacheCompleto['escutainicial']
...
try {
    $dados = listarEscutaInicialUltimos12Meses($filtros);
    ...
    error_log('ERRO ESCUTA INICIAL: ' . $e->getMessage());
    echo json_encode([
        'status' => 'error',
