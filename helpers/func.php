...
function getConsultasCacheFull() {
    static $cache = null;
    ...
    $arquivo = __DIR__ . '/CORE/cache/consultas_full.json';

    if (!file_exists($arquivo)) {
        return [];
    }

    $cache = json_decode(file_get_contents($arquivo), true);
...
function getCacheOuBanco($tipo, $distrito, $callback) {

    $arquivo = __DIR__ . "/../cache/distrito_{$distrito}/{$tipo}.json";
    ...
    if (file_exists($arquivo)) {
        ...
        if (time() - filemtime($arquivo) < $tempoCache) {
